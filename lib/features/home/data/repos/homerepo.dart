...
class HomeRepository {
  Future<Map<String, dynamic>> fetchHomeData() async {
    final response =
        await http.get(Uri.parse('https://student.valuxapps.com/api/home'));
    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      final List<dynamic> jsonBanner = data['data']['banners'];
      final List<dynamic> jsonProducts = data['data']['products'];

      final banners = jsonBanner
          .map((json) => BannerModel.fromJson({'image': json['image']}))
          .toList();
      final products = jsonProducts.map((json) {
        String englishName = _translateToEnglish(json['name']);
        String englishDescription = _translateToEnglish(json['description']);
